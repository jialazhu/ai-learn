...
MODEL_NAME = "openai/clip-vit-base-patch32"  # 请在此处填写模型名称

# TODO 1.2: 加载CLIP模型
# 提示：使用 CLIPModel.from_pretrained()
model = CLIPModel.from_pretrained(MODEL_NAME)  # 请在此处加载模型

# TODO 1.3: 加载CLIP处理器
# 提示：使用 CLIPProcessor.from_pretrained()
processor = CLIPProcessor.from_pretrained(MODEL_NAME)  # 请在此处加载处理器
...
inputs = processor(text=text_labels,images=image,return_tensors="pt", padding=True)  # 请在此处处理输入数据

# TODO 2.2: 使用模型进行推理（记得使用 torch.no_grad()）
# 提示：outputs = model(**inputs)
with torch.no_grad():
    outputs = model(**inputs)  # 请在此处进行推理

# TODO 2.3: 获取图像与文本的相似度分数
# 提示：使用 outputs.logits_per_image
logits_per_image = outputs.logits_per_image  # 请在此处获取logits

# TODO 2.4: 将logits转换为概率分布
# 提示：使用 softmax 函数，dim=1
probs = logits_per_image.softmax(dim=1)  # 请在此处计算概率
...
inputs = processor(text=[query_text],images=images,return_tensors="pt", padding=True)  # 请在此处处理输入
...
logits_per_text = outputs.logits_per_text  # 请在此处获取logits

# TODO 3.4: 转换为概率分布
probs = logits_per_text.softmax(dim=1)  # 请在此处计算概率
...
with torch.no_grad():
    image_inputs = processor(images=[query_image],return_tensors="pt")  # 请在此处处理图像输入
    image_features = model.get_image_features(**image_inputs)  # 请在此处提取图像特征
    image_features = image_features / image_features.norm(dim=-1, keepdim=True)  # 请在此处归一化特征向量
...
#   3. 归一化特征向量
with torch.no_grad():
    text_inputs = processor(text = sample_texts, return_tensors="pt", padding=True)  # 请在此处处理文本输入
    text_features = model.get_text_features(**text_inputs)  # 请在此处提取文本特征
    text_features = text_features / text_features.norm(dim=-1,keepdim= True)  # 请在此处归一化特征向量

# TODO 4.3: 计算相似度矩阵
# 提示：相似度 = image_features @ text_features.T
# 注意：归一化后的特征向量，点积就是余弦相似度
similarity_matrix = (image_features @ text_features.T) * 100   # 请在此处计算相似度矩阵
