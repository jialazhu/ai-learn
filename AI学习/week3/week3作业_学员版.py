...
class SimpleRNN:
    ...
    def forward(self, X):
        ...
        for t in range(seq_len):
            ...
            # np.dot(self.Wxh, X[t])
            # self.h[t+1] = None  # 请完成这行代码
            # self.h[t+1] = self.tanh(np.dot(self.Wxh , X[t]) + np.dot(self.Whh , self.h[t] ) + self.bh)
            self.h[t+1] = self.tanh(np.dot(X[t],self.Wxh) + np.dot(self.h[t] ,self.Whh) + self.bh)
            # 输出层: y_t = softmax(Why * h_t + by)
            # 提示：使用当前时刻的隐藏状态self.h[t+1]计算输出
            # self.y[t] = None  # 请完成这行代码
            self.y[t] = self.softmax(np.dot(self.h[t+1] ,self.Why ) + self.by )  # 请完成这行代码
...
def preprocess_text(text):
    ...
    # 返回处理后的文本字符串
    text = text.lower()
    return " ".join(jieba.lcut(text))  # 请完成这行代码
